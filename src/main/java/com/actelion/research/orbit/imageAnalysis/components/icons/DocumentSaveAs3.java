 ...
 */
public class DocumentSaveAs3 implements org.pushingpixels.flamingo.api.common.icon.ResizableIcon {
    ...
    public static void paint(Graphics2D g) {
        ...
        g.setPaint(new LinearGradientPaint(new Point2D.Double(28.061466217041016, 31.43134880065918), new Point2D.Double(28.061466217041016, 36.43749237060547), new float[]{0, 1}, new Color[]{new Color(0xDDDDDD), new Color(0xFDFDFD)}, NO_CYCLE, SRGB, new AffineTransform()));
        g.fill(shape);
        g.setPaint(new Color(0x7D7D7D));
        g.setStroke(new BasicStroke(1, 1, 0, 4));
        g.draw(shape);
        g.setComposite(AlphaComposite.getInstance(3, 0.59659094f * origAlpha));

        // _0_1_5
        shape = new Rectangle2D.Double(7, 33, 16, 7);
        g.fill(shape);
        g.setComposite(AlphaComposite.getInstance(3, 1 * origAlpha));

        // _0_1_6
        shape = new Rectangle2D.Double(24, 32, 1, 9);
        g.setPaint(BLACK);
        g.fill(shape);
