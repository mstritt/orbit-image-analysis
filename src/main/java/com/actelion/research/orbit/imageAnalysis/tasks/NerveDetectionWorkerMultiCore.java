...
/**
 * does not work properly -> use NerveDetectionWorker (single-core)
 */
@Deprecated
public class NerveDetectionWorkerMultiCore extends OrbitWorker {

    private Logger logger = LoggerFactory.getLogger(NerveDetectionWorkerMultiCore.class);
     ...
     */
    public NerveDetectionWorkerMultiCore(RecognitionFrame rf, Shape roi, List<SwingWorker<Void, Void>> dependencyList, List<ClassShape> classShapeToSet, boolean orderPoints, List<Point> tiles) {
        this.rf = rf;
    ...
    protected void doWork() throws OrbitImageServletException {
        ...
        try {
            ...
            fullRoi = roi;
            if (fullRoi == null) fullRoi = rf.getROI().getScaledInstance(100d, new Point(0, 0));

            IScaleableShape oldROI = rf.getROI();
            if (tiles == null && getSize() > 1500 * 1500L) {
                // tile list
                tiles = Arrays.asList(rf.bimg.getImage().getTileIndices(roi == null ? null : roi.getBounds()));
            }
            ...
            List<Callable<List<Shape>>> tileWorkers = new ArrayList<Callable<List<Shape>>>(tiles.size());
            final double step = /*(1d/tiles.size())*100d*/1d;
            System.out.println("step: " + step);

            for (final Point currentTile : tiles) {
                tileWorkers.add(new Callable<List<Shape>>() {
                    public List<Shape> call() throws Exception {
                        if (NerveDetectionWorkerMultiCore.this.isCancelled()) return null;
                        Shape roi = rf.bimg.getImage().getTileRect(currentTile.x, currentTile.y);
                        ...
                        if (getSize() > 5000 * 5000L) {
                            throw new IllegalArgumentException("Nucleus Segmentation can only be applied for smaller regions (max 5000x5000). Please try to select a smaller ROI and apply this method again.");
                        }


                        PlanarImage roiImage = makeROIImage(rf, roi); // needed because after dependencies (classify) have finished classImage has been changed
                        ...
                        List<Shape> detectedSegments = detectSegmentations(nerveDetectionParams.getSegmentLengthSingle(), cImg, roi);
            ...
            try {
                List<Future<List<Shape>>> tileSegments = executor.invokeAll(tileWorkers);
                for (Future<List<Shape>> future : tileSegments) {
                    if (!future.isDone()) continue;
                    List<Shape> tileSegs = future.get();
                    if (tileSegs != null)
                        segments.addAll(tileSegs);
                }
            } finally {
                executor.shutdownNow();
            }

            int allSegmentCount = segments.size();
    ...
    private List<Shape> filterOnlyCrossing(List<Shape> segments, List<ImageAnnotation> annotations) {
        ...
        for (Shape shape : segments) {
            ...
            if (isCrossing(poly, junction, nerveDetectionParams.getSegmentLengthSingle() / 2d, preAlloc)) {
                filtered.add(poly);
            }
        }
    ...
    public List<Shape> findPolygons(short[][] smap, boolean orderPoints, final int minSegmentationSize, Shape roi) {
        // find objects
        List<List<Point>> pListList = new ArrayList<List<Point>>();
        for (int x = 0; x </*classImage.getWidth()*/smap.length; x++) {
            if (isCancelled()) break;
            for (int y = 0; y </*classImage.getHeight()*/smap[0].length; y++) {
                if (isCancelled()) break;
                //System.out.println("test: "+x+","+y);
                if ((smap[x][y] > 0) && (smap[x][y] != Short.MAX_VALUE)) {
                    //System.out.println("start with: "+x+","+y);
                    if (adjoinsClass(x, y, smap)) {  // is border
                        //System.out.println("is border: "+x+","+y);
        ...
        int roiOffsX = Math.max(roi.getBounds().x, 0);
        int roiOffsY = Math.max(roi.getBounds().y, 0);
    ...
    /**
     * Returns a list of polygons. Because List<Polygon> is not an instance of List<Shape> we don't use gnerice here.
