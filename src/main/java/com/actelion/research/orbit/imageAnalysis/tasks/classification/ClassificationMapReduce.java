...
public class ClassificationMapReduce implements IMapReduce<String, Integer, ClassificationResult> {
    ...
    private final transient static Logger logger = LoggerFactory.getLogger(ClassificationMapReduce.class);
    ...
    public List<KeyValue<Integer, ClassificationResult>> map(String element) {
        ...
        try {
            ...
            ClassificationResult results = OrbitHelper.Classify(imageTiles.getRawDataFileId(), localModel, imageTiles.getTileList(), 1);
            ratio.add(new KeyValue<Integer, ClassificationResult>(imageTiles.getRawDataFileId(), results));
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("ClassificationMapReduce error: " + e.getMessage() + " / cause: " + e.getCause());
        } finally {
            try {
