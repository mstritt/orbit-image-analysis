...
package com.actelion.research.orbit.imageAnalysis.segmenter.celldetector.polygon.voronoi;
...
public class Voronoi {
   ...
   * @param yValuesIn
   *          Array of Y values for each site. Must be identical length to yValuesIn
   * @param minX
  ...
  private Edge bisect(Site s1, Site s2) {
    ...
    adx = dx > 0 ? dx : -dx;
    ady = dy > 0 ? dy : -dy;
    newedge.c = s1.coord.x * dx + s1.coord.y * dy + (dx * dx + dy * dy) * 0.5;// get the slope of
                                                                              // the line
