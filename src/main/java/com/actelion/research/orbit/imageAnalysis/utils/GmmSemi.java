...
/**
 *
 * Gaussian-Mixture-Model (GMM) Semi-Supervised Learning based on the code of MultivariateNormalMixtureExpectationMaximization.
 * See GmmSemiDemo how to use it.
 * Manuel, 2018
 *
 *
...
public class GmmSemi {
    ...
    /**
     * Labels for each observation. NA if unknown.
     */
    ...
    public void fit(final MixtureMultivariateNormalDistribution initialMixture,
        ...
        while (numIterations++ <= maxIterations &&
            ...
            for (int i = 0; i < n; i++) {
                final double rowDensity = Double.isNaN(labels[i]) ? fittedModel.density(data[i]) : 1;
                sumLogLikelihood += FastMath.log(rowDensity);

                for (int j = 0; j < k; j++) {
                    final double density = Double.isNaN(labels[i]) ? mvns[j].density(data[i]) : (((int)labels[i])==j ? 1: 0);
                    gamma[i][j] = weights[j] * density / rowDensity;
...
//                        newCovMatArrays[j][c1][c1] += random.nextDouble()/0.00001d;
//                }
