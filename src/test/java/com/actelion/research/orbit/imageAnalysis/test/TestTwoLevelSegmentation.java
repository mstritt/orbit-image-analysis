 ...
 */
public class TestTwoLevelSegmentation {
    ...
    public void testObjectSegmentation() throws OrbitImageServletException {
        OrbitModel model = OrbitModel.LoadFromInputStream(this.getClass().getResourceAsStream("/resource/testmodels/twoLevelSeg.omo"));
        RecognitionFrame rf = new RecognitionFrame("/resource/testimages/twoLevelSeg.jpg", true);
        SegmentationResult res = OrbitHelper.Segmentation(rf, -1, model, null, -1, false);
        assertNotNull(res);
        assertEquals(4, res.getObjectCount());
        assertEquals(10, res.getSecondaryObjectCount());
    }
